...
class AuthViewModel: ViewModelAdapter {
	...
	// MARK: ================ Social login ================
	func login(by userName: String, password: String) -> Observable<UserData> {
		fatalError("Implement in subclass")
	}
	
	func loginByFacebook(token: String) -> Observable<UserData> {
		fatalError("Implement in subclass")
	}
	
	func loginByGoogle(user: GIDGoogleUser) -> Observable<UserData> {
		fatalError("Implement in subclass")
	}
}
...
private extension AuthViewModel {
	...
	func proxylogin<T>(with loginParam: LoginParams<T>) -> Observable<Dictionary<String, Any>?> {
		...
		case .facebook:
			if let param = param as? FacebookParam {
