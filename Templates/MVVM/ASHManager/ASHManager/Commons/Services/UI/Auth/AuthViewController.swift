...
class AuthViewController: ViewControllerAdapter {
	...
	@IBOutlet private weak var facebookButton: GKSocialButton!
	@IBOutlet private weak var googleButton: GKSocialButton!
	...
	override func setupViews() {
		...
		emailTextField.configAttributed(placeholder: LocalizedString.titleInputUsernameOrEmail())
		passwordTextField.configAttributed(placeholder: LocalizedString.titleInputPassword())
...
private extension AuthViewController {
	...
	func signInByAccount() {
		let param = LoginParams<Void>.init(param: ())
		signIn(with: param)
	}
	...
	func signInByFacebook() {
		...
		let logInFbResult = { [weak self] (result: FBSDKLoginManagerLoginResult?, error: Error?) in
			...
			guard let fb = result, fb.isCancelled, let token = fb.token.tokenString else {
				return
			}
			
			let param = LoginParams<CommonParam>.init(param: CommonParam(token: token))
	...
	func signInByGoogle() {
		...
		GIDSignIn.sharedInstance().clientID = ServiceInfo.GOOGLE_CLIENT_ID
...
extension AuthViewController: GIDSignInDelegate, GIDSignInUIDelegate {
	func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!, withError error: Error!) {
		...
		let param = LoginParams<GoogleParam>.init(param: GoogleParam(user: user))
		self.signIn(with: param)
	}
...
class CheckBoxButton: UIButton {
	...
	private var isChecked: Bool = false {
		didSet {
			let name = isChecked ? "ic_checked" : "ic_unchecked"
...
class TermOfUseTextView: UITextView {
	...
	override func awakeFromNib() {
		...
		let policyLinkAttributes = [
			NSAttributedStringKey.link: URL(string: TermLink.policy.rawValue)!,
		...
		let termLinkAttributes = [
			NSAttributedStringKey.link: URL(string: TermLink.term.rawValue)!,
			NSAttributedStringKey.font: UIFont.boldSystemFont(ofSize: 12),
			NSAttributedStringKey.underlineStyle: NSUnderlineStyle.styleSingle.rawValue,
			NSAttributedStringKey.foregroundColor: UIColor.white
			] as [NSAttributedStringKey : Any]
		...
		attributedString.setAttributes(policyLinkAttributes, range: NSRange(location: policyIndex, length: policyString.length))
		attributedString.setAttributes(termLinkAttributes, range: NSRange(location: termIndex, length: termString.length))
