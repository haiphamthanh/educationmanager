...
extension UIView {
	...
	func addGradient(colors: [CGColor] = [MyColors.gradientColor.left.cgColor,
										  MyColors.gradientColor.right.cgColor],
					 with type: GradientType = .main) {
	...
	func addGradientBottomRightTopLeft(colors: [CGColor] = [MyColors.gradientColor.left.cgColor,
															MyColors.gradientColor.right.cgColor],
	...
	private class func loadNib<T: UIView>(_ viewType: T.Type, with frame: CGRect) -> T {
		let className = String.className(viewType)
		let nibFile = Bundle(for: viewType).loadNibNamed(className, owner: nil, options: nil)!.first as! T
		nibFile.frame = frame
