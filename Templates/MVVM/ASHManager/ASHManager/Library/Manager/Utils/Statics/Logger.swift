...
class Logger {
	...
	lazy fileprivate var fileHandle: FileHandle? = {
		...
		do {
			...
			DDLogDebug("Successfully logging to: \(path)")
			return fileHandle
		} catch let error as NSError {
			DDLogError("Serious error in logging: could not open path to log file. \(error).")
		}
...
private extension Logger {
	...
	func printToDestination(_ logMessage: String) {
		if let data = logMessage.data(using: String.Encoding.utf8) {
			...
			DDLogError("Serious error in logging: could not encode logged string into data.")
		}
	}
}
