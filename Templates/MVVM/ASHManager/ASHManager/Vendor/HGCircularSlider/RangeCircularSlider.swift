...
public class RangeCircularSlider: CircularSlider {
    ...
    */
    ...
    override open func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
		...
		// TODO: Refactor later. HaiPT15
		delta = value - oldValue
		if abs(value - oldValue) > 5000 && abs(value - oldValue) < 81400 {
			return true
		}
        if selectedThumb.isStart {
			let duration = endPointValue - startPointValue > 0 ? endPointValue - startPointValue : endPointValue - startPointValue + 86400
			let newDuration = endPointValue - value > 0 ? endPointValue - value : endPointValue - value + 86400

			if duration <= 3600 && value - oldValue > 0  {
				if endPointValue <= 3600 {
					startPointValue = endPointValue - 3600 + 86400
					sendActions(for: .valueChanged)
					return true
				}
				startPointValue = endPointValue - 3600
				sendActions(for: .valueChanged)
				return true
			}
			if duration >= 86400 - 3600 && value - oldValue < 0 {
				if endPointValue >= 86400 - 3600 {
					startPointValue = endPointValue + 3600 - 86400
					sendActions(for: .valueChanged)
					return true
				}
			...
			if newDuration < 3600 || newDuration > 86400 - 3600 {
				sendActions(for: .valueChanged)
				return true
			}
			
			if delta > 82800 {
				startDay =  Date(timeIntervalSinceReferenceDate: startDay.timeIntervalSinceReferenceDate - Double(maximumValue))
			}
			if delta < -82800 {
				startDay =  Date(timeIntervalSinceReferenceDate: startDay.timeIntervalSinceReferenceDate + Double(maximumValue))
			}
			...
			let duration = endPointValue - startPointValue > 0 ? endPointValue - startPointValue : endPointValue - startPointValue + 86400
			let newDuration = value - startPointValue > 0 ? value - startPointValue : value - startPointValue + 86400
			
			if duration <= 3600 && value - oldValue < 0  {
				if startPointValue <= 3600 {
					endPointValue = startPointValue - 3600 + 86400
					sendActions(for: .valueChanged)
					return true
				}
				endPointValue = startPointValue + 3600
				sendActions(for: .valueChanged)
				return true
			}
			if duration >= 86400 - 3600 && value - oldValue > 0 {
				if startPointValue >= 86400 - 3600 {
					endPointValue = startPointValue + 3600 - 86400
					sendActions(for: .valueChanged)
					return true
				}
			...
			let bonus: CGFloat = endPointValue - startPointValue < 0 ? 86400 : 0
			if value - oldValue > 0 {
				let date = startDay
				let currentInterval = TimeInterval(endPointValue)
				let timeZone: Double = Double(TimeZone.autoupdatingCurrent.secondsFromGMT())
				if (date!.timeIntervalSinceReferenceDate + Double(bonus) + currentInterval) >= (Date().timeIntervalSinceReferenceDate + timeZone) {
					endPointValue = CGFloat(Date().addingTimeInterval(timeZone).timeIntervalSince(DateTimeUtil.dateUTCFormatter.date(from: DateTimeUtil.dateUTCFormatter.string(from: Date()))!))
					sendActions(for: .valueChanged)
					return true
				}
			}
    ...
    // MARK: - Helpers
	func thumb(for touchPosition: CGPoint) -> SelectedThumb {
