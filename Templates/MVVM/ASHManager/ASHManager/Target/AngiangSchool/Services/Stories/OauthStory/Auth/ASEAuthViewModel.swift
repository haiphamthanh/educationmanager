...
// MARK: - ########################## Customization ##########################
class ASEAuthViewModel: AuthViewModel {
	// MARK: ================ Social login ================
	override func login(by userName: String, password: String) -> Observable<UserData> {
		let input = InNWSignIn(userName, password)
		let out = network.signIn(input: input)
	...
	override func loginByFacebook(token: String) -> Observable<UserData> {
		let input = InNWSignInByFacebook(token)
		let out = network.signInByFacebook(input: input)
	...
	override func loginByGoogle(user: GIDGoogleUser) -> Observable<UserData> {
		guard let token = user.authentication.accessToken else {
			return Observable.never()
		}
		
		let input = InNWSignInByGoogle(token)
		let out = network.signInByGoogle(input: input)
...
// MARK: - ================================= Private =================================
private extension ASEAuthViewModel {
